...
```sh
task verify         # lint + build + test + e2e (run before committing)
task build          # build into bin/nebula-pki
task test           # unit tests
task test:cover     # unit tests with coverage report
task e2e            # end-to-end testscript suite (depends on build)
task lint           # go vet + gofmt check
task fmt            # format all Go source files
task tidy           # go mod tidy
task clean          # remove build artifacts (bin/)
...
```sh
task smoke          # brew + nix installer smoke tests (isolated containers)
```
...
test/e2e/             # testscript-based end-to-end tests (task e2e)
scripts/smoke-test/   # release installer smoke tests (task smoke)
