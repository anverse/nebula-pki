...
3. **Version is observable.** The pinned upstream version is:
    ...
    * embedded in the manifest under `tool.nebula_library_version` (see [ADR-002](./002-state-and-artifact-layout.md) for the manifest schema; this field is additive and does not require a `schema_version` bump).
...
* The manifest carries enough information to reproduce a build (`tool.version` + `tool.nebula_library_version`).
...
* [ADR-002](./002-state-and-artifact-layout.md) — Manifest schema (where `tool.nebula_library_version` lives).
